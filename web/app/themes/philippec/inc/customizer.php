 ...
 */
function philippec_customize_register($wp_customize)
{
	...
	$wp_customize->add_section(
		'theme_options',
		array(
			'title' => __('Theme Options', 'philippec'),
	...
	$wp_customize->add_setting(
		...
		array(
			'default' => 'two-column',
			'sanitize_callback' => 'philippec_sanitize_page_layout',
			'transport' => 'postMessage',
		)
	);
	...
	$wp_customize->add_control(
		'page_layout',
		array(
			'label' => __('Page Layout', 'philippec'),
			'section' => 'theme_options',
			'type' => 'radio',
			'description' => __('When the two-column layout is assigned, the page title is in one column and content is in the other.', 'philippec'),
			'choices' => array(
				'one-column' => __('One Column', 'philippec'),
				'two-column' => __('Two Column', 'philippec'),
			),
			'active_callback' => 'philippec_is_view_with_layout_option',
		)
	);
	 ...
	 */
	$num_sections = apply_filters('philippec_front_page_sections', 4);
	...
	for ($i = 1; $i < (1 + $num_sections); $i++) {
		...
		$wp_customize->add_control(
			...
			array(
				...
				'label' => sprintf(__('Front Page Section %d Content', 'philippec'), $i),
				'description' => (1 !== $i ? '' : __('Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'philippec')),
				'section' => 'theme_options',
				'type' => 'dropdown-pages',
				'allow_addition' => true,
				'active_callback' => 'philippec_is_frontpage',
			)
		);
		...
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				'selector' => '#panel' . $i,
				'render_callback' => 'philippec_front_page_section',
 ...
 */
function philippec_sanitize_page_layout($input)
{
	$valid = array(
		'one-column' => __('One Column', 'philippec'),
		'two-column' => __('Two Column', 'philippec'),
 ...
 *
 * @since PhilippeC 1.0
 * @since PhilippeC 3.3 Converted function to an alias.
 *
